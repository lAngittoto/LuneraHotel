...
<div class="flex flex-col md:flex-row gap-6 items-start md:items-center text-xl">
    ...
    <label class="flex items-center gap-3 font-semibold">
        ...
        <?php
        // Use model function to check if room has active bookings
        require_once __DIR__ . '/../Models/updateRoomsModel.php';
        $hasBooking = roomHasActiveBooking($pdo, $room['id']);
        ?>
        <input type="radio" name="status" value="Booked"
            <?= ($room['status'] === 'Booked') ? 'checked' : '' ?>
            <?= !$hasBooking ? 'disabled title="Cannot mark as booked without active bookings"' : '' ?>
            class="w-6 h-6">
    </label>
                ...
                <div class="flex flex-col md:flex-row gap-4 w-full">
                    ...
                    <div class="flex flex-col w-full">
                        ...
                        <input type="number" name="people" 
                            value="<?= htmlspecialchars($room['people']) ?>" 
                            class="border border-[#dcdcdc] p-3 rounded-xl bg-[#ebebeb] outline-0">
                    </div>
                </div>
