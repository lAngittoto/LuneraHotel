      ...
      <?php foreach ($rooms as $room): ?>
        ...
        <div class="bg-[#ffffff] border border-[#ebebeb] shadow-2xl
          ...
          <div class="flex flex-col gap-2 items-center">
            <?php $isInProgress = strtolower($room['status']) === 'in progress'; ?>
            <span class="<?= htmlspecialchars($statusClass) ?> px-4 py-2 rounded-xl text-white text-center block w-full" <?= $isInProgress ? 'style="background:#4f46e5;color:#fff;"' : '' ?>>
              <?= htmlspecialchars($room['status']) ?>
            </span>
            <?php if ($room['status'] !== 'Deactivated'): ?>
              <?php if ($room['has_cleaning_task']): ?>
                <button type="button" 
                  disabled
                  ...
                  title="Cleaning task already exists for this room">
                  <i class="fa-solid fa-check-circle"></i> Set for Cleaning
                </button>
              <?php elseif ($room['status'] === 'Dirty'): ?>
                <button type="button" 
                  ...
                  <i class="fa-solid fa-broom"></i> Cleaning
                </button>
              <?php else: ?>
                <button type="button" 
                  disabled
                  style="background-color: #9ca3af; color: #d1d5db; padding: 8px 12px; border-radius: 8px; font-size: 14px; cursor: not-allowed; border: none; width: 100%;"
                  title="Room must be marked as Dirty to create cleaning task">
                  <i class="fa-solid fa-times-circle"></i> Not Dirty
                </button>
              <?php endif; ?>
            <?php endif; ?>
          </div>          <!-- ACTION BUTTONS -->
...
<div id="cleaningPopup" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999]" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0;">
      ...
      <form id="cleaningTaskForm" onsubmit="submitCleaningTask(event)">
        ...
        <div class="mb-6">
          ...
          <textarea 
            ...
            placeholder="Enter cleaning task details..."
            required
          ></textarea>
        </div>
